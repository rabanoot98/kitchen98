...
- **פריסה רספונסיבית:** מתחת ל-`sm` (640px) פועל סרגל טאבים תחתון קבוע (72px) + FAB לפעולה
  הראשית; מ-`sm` ומעלה נשאר הניווט העליון. המסכים עצמם משותפים — רק ה-chrome מתחלף.
- **מסך "עוד"** (`/more`) קיים בשביל המובייל: מרכז את מסכי המשנה (קטגוריות, סוגי קטניות,
  נמענים), את מחיקת היסטוריית הלוג ואת היציאה — כל מה שלא נכנס לחמשת הטאבים.
